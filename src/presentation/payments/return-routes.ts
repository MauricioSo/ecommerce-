...
import { getPaymentProvider } from "../../application/payments/provider.ts";
import { finalizePaymentAttemptFromProvider } from "../../application/payments/finalize.ts";
import * as repo from "../../infrastructure/payments/repository.ts";
...
export const paymentReturnRoutes = new Elysia({ prefix: "/checkout/return" })
  .get("/mercadopago", async ({ query }) => {
    ...
    if (status === "success" && paymentId) {
      ...
      if (provider.fetchExternalStatus) {
...
async function handleWebPayReturn(params: Record<string, string>) {
  ...
  const provider = await getPaymentProvider();
  if (!provider.commitReturn) {
    return redirectSuccess(attempt.orderId);
  }

  const result = await provider.commitReturn(tokenWs);
  if (!result) {
