...
import * as uc from "../../application/checkout/use-cases.ts";
import { calculateShipping } from "../../application/pricing/shipping-use-cases.ts";
import { calculateTax } from "../../application/pricing/tax-use-cases.ts";
...
import { getAddresses, getCustomerProfile } from "../../application/customers/account-use-cases.ts";
...
export const checkoutStorefrontRoutes = new Elysia()
  ...
  .post("/checkout/confirmar", async ({ body, cookie }) => {
    ...
    try {
      ...
      const paymentRepo = await import("../../infrastructure/payments/repository.ts");
