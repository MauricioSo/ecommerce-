import { findCustomerById, deleteAddressesByCustomerId, deleteAllCustomerSessions, deleteConsentRecordsByCustomerId } from "../../infrastructure/customers/repository.ts";
...
export async function exportCustomerDataUseCase(customerId: string): Promise<Record<string, unknown>> {
  ...
  const { findAddressesByCustomerId } = await import("../../infrastructure/customers/repository.ts");
  const addresses = await findAddressesByCustomerId(customerId);

  const { findConsentRecordsByCustomerId } = await import("../../infrastructure/customers/repository.ts");
