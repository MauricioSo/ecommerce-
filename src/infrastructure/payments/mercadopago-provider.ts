import type { PaymentProvider, PaymentProviderResult, WebhookEvent } from "../../application/payments/provider.ts";
...
export class MercadoPagoProvider implements PaymentProvider {
  ...
  async fetchExternalStatus(paymentId: string): Promise<PaymentProviderResult | null> {
    const result = await this.fetchPaymentStatus(paymentId);
    if (!result) return null;
    return {
      success: true,
      providerIntentId: `mp_${paymentId}`,
      status: result.status as PaymentProviderResult["status"],
